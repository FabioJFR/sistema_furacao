...
def _analisar_relatorio(analise):
    ...
    for indice, zona in enumerate(zonas, start=1):
        ...
        DeteccaoImagemAI.objects.create(
            ...
            texto_sugerido=deteccao["valor_sugerido"],
    ...
    texto_sugerido_total = "\n".join(
        f"{item['rotulo']}: {item['valor_sugerido']}" for item in deteccoes
    )
...
def _construir_zonas_relatorio(*, largura, altura):
    ...
    return [
        {
            "rotulo": "Cabeçalho impresso",
            "tipo_zona": "cabecalho_impresso",
            "tipo_conteudo": "impresso",
            "x_min": 0,
            "x_max": largura,
            "y_min": 0,
            "y_max": int(topo_h * 0.38),
        },
        {
            "rotulo": "Campos manuais superiores",
            "tipo_zona": "campos_manuais_superiores",
            "tipo_conteudo": "manual",
            "x_min": 0,
            "x_max": largura,
            "y_min": int(topo_h * 0.38),
            "y_max": topo_h,
        },
        {
            "rotulo": "Zona central manuscrita",
            "tipo_zona": "zona_central_manual",
            "tipo_conteudo": "manual",
            "x_min": int(largura * 0.05),
            "x_max": int(largura * 0.95),
            "y_min": topo_h,
            "y_max": topo_h + corpo_h,
        },
        {
            "rotulo": "Rodapé impresso",
            "tipo_zona": "rodape_impresso",
            "tipo_conteudo": "impresso",
            "x_min": 0,
            "x_max": largura,
            "y_min": topo_h + corpo_h,
            "y_max": topo_h + corpo_h + rodape_h,
        },
    ]


def _avaliar_zona_relatorio(*, pixels, largura, altura, zona):
    ...
    valor_sugerido = _valor_sugerido_relatorio(zona, densidade, densidade_azul)
    return {
        "rotulo": zona["rotulo"],
...
def _valor_sugerido_relatorio(zona, densidade, densidade_azul):
    if zona["tipo_zona"] == "cabecalho_impresso":
        return "Campo impresso de identificação do relatório"
    if zona["tipo_zona"] == "campos_manuais_superiores":
        if densidade_azul > 0.0015:
            return "Campos superiores preenchidos manualmente, provável data/turno/equipa"
        return "Zona superior para preenchimento manual"
    if zona["tipo_zona"] == "zona_central_manual":
        if densidade_azul > 0.0015:
            return "Corpo principal do relatório com texto manuscrito"
        return "Zona central preparada para observações manuscritas"
    return "Rodapé impresso com informação fixa ou validação"
...
def _analisar_caixa_cilindrica(analise):
    with Image.open(analise.imagem_original) as imagem:
        ...
        resultado = _executar_pipeline_caixa(imagem_corrigida)
        ...
        for indice, deteccao in enumerate(resultado["deteccoes_zonas"], start=1):
            ...
            DeteccaoImagemAI.objects.create(
                ...
                confianca=deteccao["confianca"],
                texto_sugerido=deteccao["texto_sugerido"],
                caixa_delimitadora=bbox,
                metadados=deteccao["metadados"],
            )
            if not maior_bbox:
...
def _executar_pipeline_caixa(imagem):
    largura, altura = imagem.size
    pixels = imagem.load()
    pontos_marcador = []
    pontos_azul = 0
...
def _avaliar_zona(*, pixels, largura, altura, passo, zona):
    x_min = max(0, int(zona["x_min"]))
    x_max = min(largura, int(zona["x_max"]))
    y_min = max(0, int(zona["y_min"]))
    y_max = min(altura, int(zona["y_max"]))
    ...
    texto_sugerido = _texto_sugerido_zona(zona)
