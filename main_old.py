...
# ================= CALLBACK para o mapa de projetos=================
@app.callback(
    Output("url", "pathname"),
    Input("mapa-projetos", "clickData"),
    State("store-projetos", "data"),
    prevent_initial_call=True
)
def navegar_projeto(clickData, data):
    """
    Ao clicar em um projeto no mapa global, redireciona para o mapa local do projeto.
    """
    if not clickData:
        return dash.no_update
    ...
    try:
        proj_id = clickData["points"][0]["customdata"][0]  # id do projeto
        for p in projetos:
            if p.id == proj_id:
                return f"/projeto/{p.id}"
    except Exception as e:
        print("Erro no callback do mapa de projetos:", e)
        return dash.no_update

    return dash.no_update

#================== Callback para o mapa de furos=================
@app.callback(
    Output("url", "pathname"),
    Input("mapa-furos", "clickData"),
    State("store-projetos", "data"),
    prevent_initial_call=True
)
def navegar_furo(clickData, data):
    """
    Ao clicar em um furo no mapa local do projeto, redireciona para o detalhe do furo.
    """
    ...
    try:
        furo_id = clickData["points"][0]["customdata"][0]  # id do furo
        for p in projetos:
            for f in p.furos:
                if f.id == furo_id:
                    return f"/furo/{f.id}"
    except Exception as e:
        print("Erro no callback do mapa de furos:", e)
        return dash.no_update
